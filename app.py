...
def graph_info(ax, window_size):
    ...
    # Microbes
    for microbe in microbes:
        ax[0].plot(range(len(microbe.pop_history)), microbe.pop_history, label=microbe.name)
    ...
    # Carrying capacity
    for microbe in microbes:
        ax[1].plot(range(len(microbe.k_history)), microbe.k_history, label=microbe.name)
